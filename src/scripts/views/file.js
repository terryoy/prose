...
module.exports = Backbone.View.extend({
  ...
  showDiff: function() {
    ...
    var text1 = this.model.isNew() ? '' : escape(this.model.get('previous'));
    var text2 = escape(this.model.serialize());
  ...
  absolutePathFromComponents: function(user, repo, branch, path) {
    var url = compact([ user, repo, branch, path ]);
    return url.join('/');
  },
    ...
    method.call(this, {
      success: (function(model, res, options) {
        ...
        if (model.previous('sha') && pathChange) {
          ...
          params = map(pairs(data), function(param) {
            return param.join('=');
          }).join('&');
