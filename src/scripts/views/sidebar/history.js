...
module.exports = Backbone.View.extend({
    ...
    renderFiles: function(commits, label) {
        ...
        var files = flatten(map(commits, function(commit) {
    ...
    */
        ...
        var fileCommitsMap = groupBy(files, function(file) {
            return file.filename;
        });

        var list = uniq(map(files, function(file) {
            return file.filename;
        }));
        ...
        if (list.length) {
            ...
            list.slice(0,5).forEach((file, index) => {
                var commits = fileCommitsMap[file];
            ...
            var tmpl = template(this.template, {variable: 'label'});
    ...
    render: function(options) {
        ...
        union(this.history, this.recent).forEach(function(commit) {
            q.defer(function(cb) {
                commit.fetch({
