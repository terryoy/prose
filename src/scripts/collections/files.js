...
module.exports = Backbone.Collection.extend({
  ...
  parse: function(resp, options) {
    return map(resp.tree, (function(file) {
      return  extend(file, {
        branch: this.branch,
        collection: this,
        repo: this.repo
  ...
  parseConfig: function(config, options) {
    ...
    if (config && config.prose) {
      ...
      this.config = extend(config, config.prose);
      ...
      if (config.prose.metadata) {
        ...
        forEach(metadata, function(raw, key) {
          q.defer(function(cb) {
            var subq = queue();
            var defaults;
        ...
        q.awaitAll((function() {
          ...
          if (isFunction(options.success)) options.success.apply(this, options.args);
        }).bind(this));
      } else {
        if (isFunction(options.success)) options.success.apply(this, options.args);
      }
    ...
    Backbone.Collection.prototype.fetch.call(this, extend(options, {
      success: (function(model, res, options) {
        ...
        if (config) {
          ...
          if (isFunction(success)) success.apply(this, args);
        }
      ...
      success: (function(res) {
        ...
        model.save({
          success: (function(model, res, options) {
            ...
            if (isFunction(success)) success(model, res, options);
          }).bind(this),
          error: options.error
        });
      }).bind(this),
